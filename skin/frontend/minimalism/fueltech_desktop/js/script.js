...
/* Footer Map button */
function mapButton(){
	button = jQuery('#footer #find-us');
	if(button.length){
		button.css('margin-left', '-' + button.width()/2 + 'px');
	}
}
...
function topCartListener(e){
	...
	if(jQuery(touch.target).parents('#topCartContent').length == 0 && jQuery(touch.target).parents('#cartHeader').length == 0 && !jQuery(touch.target).attr('id') != '#cartHeader'){
		...
		jQuery('#topCartContent').hide().removeClass('active');
...
function topCart(){
	jQuery('.top-cart #cartHeader').on('click', function(event){
		...
		jQuery('#topCartContent').toggle().toggleClass('active');
		...
		setTimeout(function(){
			...
			switch(mode)
			{
				case 'on':
					...
					left_indent = search.offset().left + search.outerWidth(true) - 12;
					top_cart.css({
						'top': top_indent,
						'left': left_indent,
						'opacity': 1
...
jQuery(window).load(function() {
  ...
  /* Coin Slider */
	...
	function accordion (status){
		if(status == 'enable'){
			jQuery('.footer-columns-block h3').on('click', function(){
				jQuery(this).toggleClass("active").parent().find(".custom-footer-content, .block-subscribe").slideToggle('medium');
				wishlist_slider();
			})
			jQuery('.footer-columns-block').addClass('accordion').find(".custom-footer-content, .block-subscribe").slideUp('fast');
		}else{
			jQuery('.footer-columns-block h3').removeClass("active").off().parent().find(".custom-footer-content, .block-subscribe").slideDown('fast');
	...
	mapButton();
	//menuHeight2();
	jQuery(window).resize(function(){toDo(); mapButton(); replacingClass(); more_view_set_height(); wishlist_set_height(); related_set_height(); resize_window(); topSearch(); menuPosition();});
	/* Responsive */
...
/* Messages button */
	...
	if(jQuery('body').hasClass('floating-header')){
		...
		toplineHeight = jQuery('.topline').height();
		jQuery(window).scroll(function(){
			if(jQuery(this).scrollTop() >= headerHeight - toplineHeight ){
